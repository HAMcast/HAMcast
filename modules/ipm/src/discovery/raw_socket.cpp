...
#include "discovery/raw_socket.hpp"
...
bool raw_socket::bind_Raw_Socket() {
     HC_LOG_TRACE("");
     if (!is_raw_valid()) {
          HC_LOG_ERROR("bind_Raw_Socket:raw_socket invalid");
          return false;
     }
     sockaddr_in m_addr;
     int rc;
     ...
     if (rc == -1) {
          HC_LOG_ERROR("bind_Raw_Socket: failed to bind! Error: " << strerror(errno));
          return false;
     } else {
          return true;
     }
...
bool raw_socket::setLoopBack(bool enabled) {
     HC_LOG_TRACE("");
     if (!is_raw_valid()) {
          HC_LOG_ERROR("receive_Raw_Packet:raw_socket invalid");
          return false;
     }
     ...
     if (rc == -1) {
          HC_LOG_ERROR("setLoopBack:failed to setLoopBack(on/off)! Error: " << strerror(errno));
          return false;
     } else {
          return true;
     }
