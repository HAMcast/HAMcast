...
void mroute_socket::test_add_vifs(mroute_socket* m){
     ...
     //string str_if_three = MROUTE_SOCKET_IF_STR_THREE;
     ...
     if(m->addVIF(if_one, str_if_one.c_str(),false,false,false,NULL)){
          cout << "addVIF " << str_if_one << " OK!" << endl;
     }else{
          cout << "addVIF " << str_if_one << " FAILED!" << endl;
     }

     if(m->addVIF(if_two, str_if_two.c_str(),false,false,false,NULL)){
...
void mroute_socket::test_add_route(mroute_socket* m){
     ...
     if(m->addMRoute(if_one, src_addr, g_addr ,output_vifs, sizeof(output_vifs)/sizeof(output_vifs[0]))){
...
void mroute_socket::test_del_route(mroute_socket* m){
     ...
     if(m->delMRoute(if_one, src_addr, g_addr)){
