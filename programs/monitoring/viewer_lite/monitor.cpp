...
QList<QString> monitor::get_node_list()
{
    std::string meth = "/node_list";
    std::vector<std::string> args;
    http_message send_msg(meth,args);
    QString send_data = QString::fromStdString(send_msg.to_string());
    m_tcp_connection.send(send_data);
    ...
    QList<QString> node_list = node_list_received(pt);
    return node_list;
}

QString monitor::hamcast_node_string(QString &name)
{
    std::string meth = "/node_data";
    std::vector<std::string> args;
    std::string local_name = name.toStdString();
    ...
    std::string payload = msg.get_payload();
    QString node_data = QString().fromStdString(payload);
    return node_data;
}

QString monitor::edge_list_string(QString &group)
{
    std::string meth = "/group_tree";
    std::vector<std::string> args;
    std::string local_name = group.toStdString();
    args.push_back(local_name);
    http_message send_msg(meth,args);
    QString data = QString::fromStdString(send_msg.to_string());
    m_tcp_connection.send(data);
    ...
    http_message msg(tcp_data);
    std::string payload = msg.get_payload();
    QString tree_data = QString().fromStdString(payload);
    return tree_data;
}
...
QList<QPair<QPair<QString,QString>, QString >  > monitor::get_group_tree(QString & group_name){
    std::string meth = "/group_tree";
    std::vector<std::string> args;
    ...
    QList<QPair<QPair<QString,QString>, QString >  > group_tree = group_tree_received(pt);
    qDebug()<< group_tree;
    return group_tree;
}
...
QList<QString> monitor::node_list_received(boost::property_tree::ptree &data)
{
    QList<QString> local_list;
    try{
        BOOST_FOREACH(boost::property_tree::ptree::value_type &v,data.get_child("nodes")){
            local_list.append(QString::fromStdString(v.second.data()));
        }
    }
    catch(std::exception& e){
...
QList<QPair<QPair<QString,QString>, QString >  > monitor::group_tree_received(boost::property_tree::ptree& data){
