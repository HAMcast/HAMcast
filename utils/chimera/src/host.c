...
{
    int ret = 0;
    if (len < HOST_CODED_SIZE) {
        fprintf(stderr, "host_encode: invalid buffer lenght!\n");
        ret = -1;
    }
    else {
        char* write = s;
        write_key_to_buffer(host->key, write);
        ret += (KEY_ARRAY_SIZE*sizeof(uint32_t));
        write = s+ret;
        write_u32_to_buffer(ntohl(host->address), write);
        ret += sizeof(uint32_t); // sizeof(host->address);
        write = s+ret;
        write_u16_to_buffer(host->port,write);
        ret += sizeof(uint16_t); // sizeof(host->port);
        if (ret != HOST_CODED_SIZE) {
            fprintf(stderr, "host_encode: error!\n");
            ret = -1;
        }
    }
    return ret;
...
ChimeraHost *host_decode (ChimeraState * state, const char *s)
{
    ...
    Key k;
    const char* read = s;
    k = read_key_from_buffer(read);
    read += (KEY_ARRAY_SIZE*sizeof(uint32_t));
    addr = htonl(read_u32_from_buffer(read));
    read += sizeof(uint32_t);
    port = read_u16_from_buffer(read);

    ptr = inet_ntop(AF_INET, &addr, name, sizeof(name));
    if (ptr != NULL) {
        host = host_get (state, name, port);

        if (key_equal_ui (host->key, 0))
            key_assign (&(host->key), k);
    }
    else {
        fprintf (stderr, "host_decode; failed to convert address!\n");
    }
    return (host);
}
 ...
 */
ChimeraHost *host_get (ChimeraState * state, const char *hostname, int port)
{
    JRB node;
    Dllist dllnode;
    uint32_t address;
    ...
    inet_ntop (AF_INET, &address, id, sizeof(id));
    // append port to id
    ...
    if (node == NULL) {
        ...
        entry->host->success_avg = GOOD_LINK;
        entry->reference = 1;
...
void host_release (ChimeraState * state, ChimeraHost * host)
{
    ...
    /* create an id of the form ip:port */
    memset (id, 0, 256);
    inet_ntop (AF_INET, &(host->address), id, sizeof(id));
    // append port to id
