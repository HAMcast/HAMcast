...
JRB jrb_find_gte_dbl (JRB n, double dkey, int *fnd)
{
    *fnd = 0;
    if (!ishead (n))
	{
	    fprintf (stderr, "jrb_find_gte_dbl called on non-head 0x%p\n",
		     (void *) n);
	    exit (1);
	}
    if (n->parent == n)
...
int jrb_nblack (JRB n)
{
    ...
    {
        fprintf (stderr,
             "ERROR: jrb_nblack called on a non-external node 0x%p\n",
             (void *) n);
        exit (1);
    }
    nb = 0;
...
int jrb_plength (JRB n)
{
    ...
    {
        fprintf (stderr,
         "ERROR: jrb_plength called on a non-external node 0x%p\n",
            (void *) n);
        exit (1);
    }
    pl = 0;
