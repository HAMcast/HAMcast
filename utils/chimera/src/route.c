...
void *route_init (ChimeraHost * me)
{
    ...
    //rg->keystr = (char *) malloc (sizeof (char) * (KEY_SIZE + 1));
...
ChimeraHost **route_get_table (ChimeraState * state)
{
    ...
    for (i = 0; i < MAX_ROW; i++)
    {
        for (j = 0; j < MAX_COL; j++)
        {
            for (l = 0; l < MAX_ENTRY; l++)
            {
                if ( (routeglob->table[i][j][l] != NULL) && (routeglob->table[i][j][l]->success_avg > BAD_LINK) )
                {
                    ret[k++] = host_get (state, routeglob->table[i][j][l]->name,
			    ...
			    int is_safe)
{
    ...
    if ((count == 1) &&
        key_between(state->log, &key, &routeglob->Lrange,&routeglob->Rrange)) {
        ChimeraHost *hosts[LEAFSET_SIZE+2];
        ret = (ChimeraHost **) malloc (2 * sizeof (ChimeraHost*));
...
//    match_col = hexalpha_to_int ((get_key_string(&key))[i]);
    ...
    if (next_hop == 1 && count == 1) {
        for (k = 0; k < MAX_ENTRY; k++) {
            if (routeglob->table[i][match_col][k] != NULL) {
                if ((routeglob->table[i][match_col][k]->success_avg > tmp->success_avg) ||
                    ((routeglob->table[i][match_col][k]-> success_avg == tmp->success_avg)
                    && (routeglob->table[i][match_col][k]-> latency < tmp->latency)))
                    tmp = routeglob->table[i][match_col][k];
            }
		}
        ret = (ChimeraHost **) malloc (2*sizeof (ChimeraHost *));
    ...
    ChimeraHost *bnh[LEAFSET_SIZE + 1 + (MAX_COL * MAX_ENTRY)];
    leaf = routeglob->me;
    bnh[index++] = routeglob->me;
	...
	if (count == 1)
	{
        ret[0] = find_closest_key (state, bnh, key, index);
		ret[1] = NULL;
	}
	else
	{
        sort_hosts (state->log, bnh, key, index);
		/* find the best #count# entries that we looked at... could be much better */
        for (i = 0, j = 0; bnh[i] != NULL && (i - j) < count; i++)
		{
            tmp = bnh[i];
		     ...
		     ChimeraHost ** deleted, ChimeraHost ** added)
{
    ...
    if (joined)
	{
        ...
        if (key_between(state->log, &host->key, &routeglob->me->key, &midpoint)) {
...
ChimeraHost **route_neighbors (ChimeraState * state, int count)
{
    ...
    int ret_size;
    ChimeraHost *tmp;
    ChimeraHost *hosts[LEAFSET_SIZE + 1];
    ChimeraHost **ret =
...
void route_update (ChimeraState * state, ChimeraHost * host, int joined)
{
    ...
    j = get_col(host->key,i);//hexalpha_to_int (get_key_string (&host->key)[i]);
